...
def inference_video_vps_save_results(
    pred_ious, pred_stability_scores, pred_masks, out_size,
    overlap_threshold=0.7, mask_binary_threshold=0.5, object_mask_threshold=0.05, test_topk_per_image=100
):
    '''
    ...
    if pred_stability_scores:
        ...
        from train.utils.comm import convert_mask_to_box, calculate_mask_quality_scores
    ...
    if cur_masks.shape[0] == 0:
        ...
        cur_masks = F.interpolate(
            cur_masks, size=out_size, mode='bilinear'
        )
...
def process(video_id, frame_names, outputs, categories_dict, output_dir, video_dir):
    ...
    for i, image_name in enumerate(frame_names):
        ...
        image_.save(os.path.join(output_dir, 'pan_pred', video_id, image_name.split('/')[-1].split('.')[0] + '.png'))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run the model with a specified checkpoint directory.")
    parser.add_argument('--ckpt_dir', type=str, required=True, help="Checkpoint directory name")
    parser.add_argument('--model_cfg', type=str, default='configs/sam2.1_hiera_l.yaml', help="Base SAM 2 SModel config file")
    parser.add_argument('--mask_decoder_depth', type=int, default=8, help="Mask decoder depth")
    ...
    for cat in categories_list:
        cat['id'] = cat['id'] + 1  # start from 1
        cat['isthing'] = 1  # assume all entities
    ...
    for video_idx in tqdm(processing_order, desc="Inference VIPSeg"):
        ...
        pred_masks_list = []
        pred_eious = []
        for out_frame_idx, _, out_mask_logits, pred_eiou in predictor.propagate_in_video(inference_state,start_frame_idx=0):
        ...
        result_i = inference_video_vps_save_results(pred_eious, pred_stability_scores, pred_masks, out_size)
        anno = process(video_id, frame_names, result_i, categories_dict, output_dir, video_dir)
